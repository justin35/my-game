...
var player   = { name: "", goodpoint: 0, badpoint: 0 };
...
window.onload = function(){
    ...
    button.onclick = function(){
        ...
        document.getElementById("scorepoints").style.display="block";
      ...
      for (var i = 0; i < buttonsCount; i += 1) {
        buttons[i].onclick = function(e) {
            ...
            (fakenews.includes(this.querySelector(".title").innerHTML)) ? player.goodpoint += 1 : player.goodpoint;
            var score = document.getElementById("myscore");
        score.innerHTML = player.goodpoint;
            alert( (fakenews.includes(this.querySelector(".title").innerHTML)) ? "Gagné" : "Perdu")
            // (fakenews.includes(this.querySelector(".title").innerHTML)) ? player.score +=1 : player.score
